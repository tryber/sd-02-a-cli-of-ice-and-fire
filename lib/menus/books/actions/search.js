...
const showBooks = async (backToBooks, pageLink) => {
  const title = pageLink || await inputTitle();

  const { content, links } = await getBooks(showBooks, backToBooks, pageLink, title);
