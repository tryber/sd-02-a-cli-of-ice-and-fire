 ...
 */
const handleUserChoice = (userChoice, { goBackToBooksMenu, showBooks, links }) => {
  ...
  if (userChoice === 'next' || userChoice === 'prev') {
     ...
     */
    return showBooks(goBackToBooksMenu, links[userChoice]);
  }
...
const getBooks = async (showBooks, goBackToBooksMenu, pageLink, title) => {
  ...
  if (Object.keys(content).length === 0) {
    ...
    return showBooks(goBackToBooksMenu);
  }
...
const showBooks = async (goBackToBooksMenu, pageLink) => {
  let title;
  if (!pageLink) {
    title = await inquirer.prompt(
      { type: 'input', name: 'bookName', message: 'Qual o nome do livro que gostaria de conhecer melhor?' },
    ).then(({ bookName }) => bookName);
  }

  const { content, links } = await getBooks(showBooks, goBackToBooksMenu, pageLink, title);
