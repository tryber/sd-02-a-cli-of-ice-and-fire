...
const removePropertiesFromBooks = ({ books, povBooks, ...book }) =>
  removeEmptyProperties(book);

const createChoiceFromBooks = (book) => ({
  /* Uma livro pode não ter nome. Nesses casos, a API traz a propriedade `alias`,
         que é o que usamos aqui para mostrar a livro na lista */
 ...
 * Na primeia execução, o parâmetro `bookInput` estará vazio,
 ...
 * @param {string} bookInput Link da página a ser exibida.
 */

const showBooksList = async (goBackToBooksMenu) => {
  const bookInput = await inputBookName();
