...
const BOOKS_FROM_NAME = 'https://www.anapioficeandfire.com/api/books?name=';
...
const handleUserChoice = (userChoice, { goBackToBooksMenu, showBooksList, links }) => {
  ...
  if (userChoice === 'next' || userChoice === 'prev') {
     ...
     */
    return showBooksList(goBackToBooksMenu, links[userChoice], userChoice);
  }
...
const showBooksList = async (goBackToBooksMenu, pageLink, listBooksChoices) => {
  let bookLink;

  if (!listBooksChoices) {
    bookLink = `${BOOKS_FROM_NAME}${await inputBookName()}`;
  }

  const { books, links } = await getBooksFromPage(pageLink || bookLink || FIRST_PAGE_LINK);
  ...
  return showBooksList(goBackToBooksMenu, FIRST_PAGE_LINK, userChoice);
