...
const handleUserChoice = (userChoice,
  { goBackToBooksMenu, showBooksList, links, booksFoundArray, pageLink }) => {
  ...
  if (booksFoundArray.length === 1 && !pageLink) {
    return showBooksList(goBackToBooksMenu, returnPage, booksFoundArray);
  }
  return showBooksList(goBackToBooksMenu, returnPage);
};
...
const showBooksList = async (goBackToBooksMenu, pageLink, book) => {
  console.log('0 - Iniciando show booklist');
  const resultsObj = pageLink ? await getBook(pageLink) : await typeBookSearch(FIRST_PAGE_LINK);
  let { booksFoundArray } = resultsObj;
  const { links } = resultsObj;
  if (!booksFoundArray.length) {
    console.log('Nenhum livro encontrado para essa pesquisa');
    return goBackToBooksMenu();
  }

  booksFoundArray = book || booksFoundArray;
  const adjustedpageLink = book ? '' : pageLink;
  const adjustedLink = book ? {} : links;

  const choices = createChoicesList(booksFoundArray, adjustedLink);
