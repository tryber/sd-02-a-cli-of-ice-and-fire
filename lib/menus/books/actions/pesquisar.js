...
const handleUserChoice = async (userChoice, links, goBackToBooksMenu) => {
  if (userChoice === 'next' || userChoice === 'prev')
    return await pesquisar(goBackToBooksMenu, links[userChoice]);
...
const pesquisar = async (goBackToBooksMenu, redlinks) => {
  if (!redlinks) {
    ...
    var { links, books } = (answer) ?  
      await getBooksFromPage(`${FIRST_PAGE_LINK}&name=${answer}`) :
      await getBooksFromPage(`${FIRST_PAGE_LINK}`);
  } else {
    var { links, books } = await getBooksFromPage(redlinks);
  }
  if (books.length > 0) {
    const choices = createChoicesList(books, links);
    var userChoice = await showMenuOptions({ message: MENU_MAIN_MESSAGE, choices });
  } else {
    var userChoice = await showMenuOptions({ message: NO_BOOK_FOUND, choices: NO_BOOK_CHOICES });
  }
