...
const getHousesFromPage = async (pageLink) => {
  try {
    const { headers, body } = await superagent.get(pageLink);
    const houses = body;
...
const housesList = async (goBackToHousesMenu, pageLink) => {
  ...
  if (nextAction === 'repeat') {
    return housesList(goBackToHousesMenu);
  }
