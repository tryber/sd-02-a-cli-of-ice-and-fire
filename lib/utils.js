...
function addExtraChoices(choices, links) {
...
async function getDataFromPage(pageLink) {
...
function removeEmptyProperties(object) {
  return Object.fromEntries(Object.entries(object).filter(([_, value]) => !isEmpty(value)));
}

async function showMenuInsert({ message }) {
...
async function showMenuOptions({ message, choices }) {
  return inquirer
    .prompt({
      type: 'list',
      name: 'choice',
